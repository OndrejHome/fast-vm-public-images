...
guestfish -a "/dev/$THINPOOL_VG/$VM_NAME" -m /dev/r6vg/root_lv -m /dev/sda2:/boot <<EOF
...
if [ ! -e "/etc/fast-vm/nvram-${image_name}_VARS.fd" ]; then
	echo "[!!!] NVRAM for this image is missing. System won't be able to boot. Place the correct NVRAM into /etc/fast-vm/nvram-${image_name}_VARS.fd"
	exit 1
fi
