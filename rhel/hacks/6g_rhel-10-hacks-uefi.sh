...
if [ ! -f /var/lib/fast-vm/appliance/capability_xfs_el9 ]; then
        echo "[!!!] XFS_EL9 capable appliance required for proper run"
        exit 1
fi
...
guestfish -a "/dev/$THINPOOL_VG/$VM_NAME" -m /dev/r10vg/root_lv -m /dev/sda2:/boot -m /dev/sda1:/boot/efi --selinux <<EOF
