...
virt-edit --connect qemu:///system -d $VM_NAME -m /dev/c6vg/root_lv -m /dev/sda1:/boot /etc/sysconfig/network-scripts/ifcfg-eth0 -e "s/HWADDR=.*$/HWADDR=${vm_mac}/; s/ONBOOT=no/ONBOOT=yes/"

## hostname
VM_HOSTNAME=$(echo $VM_NAME|sed -e 's/\./-/g; s/_/-/g')
virt-edit --connect qemu:///system -d $VM_NAME -m /dev/c6vg/root_lv -m /dev/sda1:/boot /etc/sysconfig/network -e "s/HOSTNAME=.*$/HOSTNAME=$VM_HOSTNAME/"
