...
guestfish -a "/dev/$THINPOOL_VG/$VM_NAME" -m /dev/f33/root_lv -m /dev/sda1:/boot --selinux <<EOF
sh 'sed -i "s/autoconnect=false/autoconnect=true/" /etc/NetworkManager/system-connections/ens${net_card}.nmconnection'
...
selinux-relabel /etc/selinux/targeted/contexts/files/file_contexts /etc/NetworkManager/system-connections/ens${net_card}.nmconnection
