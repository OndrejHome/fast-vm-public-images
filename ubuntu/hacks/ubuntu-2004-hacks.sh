...
guestfish -a "/dev/$THINPOOL_VG/$VM_NAME" -m /dev/u2004vg/root_lv -m /dev/sda2:/boot <<EOF
...
sh 'sed -i "s/.*ethernets.*/  ethernets: { ens${net_card}: { dhcp4: yes } }/" /etc/netplan/00-installer-config.yaml'
# disable system swap file as we use the swap_lv
sh 'sed -i "/swap.img/d" /etc/fstab'
# set the hostname
